...
int assembler::assemble(std::string input_filename, std::string output_filename)
{
    ...
    for (int line_index = 0; line_index < file_content.size(); ++line_index)
    {
        ...
        if (line[0] == '.')
        {
            ...
            else if (pseudo_command == ".STRINGZ")
            {
                ...
                std::string word;
                std::getline(line_stringstream, word);
                word = Trim(word);
    ...
    if (output_filename == "")
    {
        ...
        {
            output_filename = output_filename + "s" + ".txt";
        }
        else
        {
            ...
            if (command_tag != -1)
            {
                ...
                case 23:
                    ...
                    // TO BE DONE
                    {
                        ...
                        int num_temp = RecognizeNumberValue(parameter_list[0]);
                        for (int i = 8; i <= 15; i++)
                            result_line += NumberToAssemble(num_temp)[i];
                        break;
                    }
                default:
                    // Unknown opcode
