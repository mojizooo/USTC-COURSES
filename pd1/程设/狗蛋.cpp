...
void range(int queue[], int x, int v){
	for(int i=80;i>=x;i--){
		if(!queue[i]) continue;
		queue[i+1]=queue[i];
	}
...
void candi(int num[][9]){
	memset(queue,0,sizeof(queue));
	int sum=0;
	int h=0;
	for(int i=0;i<9;i++){
		for(int j=0;j<9;j++){
			if(num[i][j]&&!candidates[i*9+j][0]){
				continue;
			}
			else{
				sum=0;
				for(int k=1;k<=9;k++){
					if(candidates[i*9+j][k]<0) continue;
					if(isright(num,i,j,k)){
						candidates[i*9+j][k]=1;
						candidates[i*9+j][0]=1;
						sum++;
					}
					else
						candidates[i*9+j][k]=0;
				}
				if(num[i][j]&&candidates[i*9+j][0]) candidates[i*9+j][num[i][j]]=0;
				if(queue[0]==0){
					queue[0]=sum*100+i*9+j;
					continue;
				}
				for(h=0;queue[h]!=0;h++){
					if(sum<=queue[h]/100) {
						range(queue, h, sum*100+i*9+j);
						break;
					}
				}
				if(!queue[h]) queue[h]=sum*100+i*9+j;
			}
		}
	}
	//for(int i=0;i<=80&&queue[i];i++) printf("%d\n",queue[i]);
} 
...
bool GBFS(int num[][9]){
	...
	int flag = 0;
	while(m>=0){
		for(i=0,m=queue[0];num[m%100/9][m%100%9]&&i<=80;i++) m = queue[i]; 
		if(i==81) return true;
		for(k=1;k<=9;k++){
			if(candidates[m%100][k]>0){
				cell[j] = m;
				//printf("%d\n",j);
				num[m%100/9][m%100%9] = k;
				//printf("%d %d\n",m%100/9,m%100%9);
				//print(num);
				//putchar('\n');
				break;
		...
		if(k==10){
			...
			int m_ = cell[--j];
			int n = num[m_%100/9][m_%100%9];
			num[m_%100/9][m_%100%9] = 0;
