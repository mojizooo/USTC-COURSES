...
int split_string(char* string, char *sep, char** string_clips) {
    ...
    string_clips[0] = strtok(string, sep);  //string is changed
    int clip_num = 0;
...
int process_redirect(int argc, char** argv, int *fd) {
    ...
    while(i < argc) {
        ...
        if(strcmp(argv[i], ">") == 0) {
            //TODO: 打开输出文件从头写入
            tfd = open(argv[i + 1], O_WRONLY|O_CREAT|O_TRUNC, S_IRWXU|S_IRWXO|S_IRWXG);
            if(tfd < 0) {
            ...
            //TODO: 打开输出文件追加写入
            tfd = open(argv[i + 1], O_WRONLY|O_CREAT|O_APPEND, S_IRWXU|S_IRWXO|S_IRWXG);
            if(tfd < 0) {
                printf("open '%s' error: %s\n", argv[i+1], strerror(errno));
            } else {
                //TODO:输出重定向
...
int main() {
    ...
    while (1) {
        ...
        cmd_count_0 = split_string(cmdline, ";", commands_0);
        if(cmd_count_0 == 0) {
            continue;
        }
        else while(i < cmd_count_0) {
        /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
        /* 拆解命令行 */
        cmd_count = split_string(commands_0[i++], "|", commands);
